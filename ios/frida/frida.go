...
func StartFrida(ctx context.Context, wr io.Writer, bundleId, code string) error {
	if strings.TrimSpace(bundleId) == "" {
		return errors.New("bundleId 不允许为空")
	}

	writer = wr

	if _, err := C.frida_init(); err != nil {
		return fmt.Errorf("frida_init error: %w", err)
	}
	...
	if gErr != nil {
		return fmt.Errorf("enumerate devices error: %s", C.GoString(gErr.message))
	}
	...
	if gErr != nil {
		return fmt.Errorf("spawn target error: %s", C.GoString(gErr.message))
	}
	hSession := C.frida_device_attach_sync(hDevice, pid, C.FRIDA_REALM_NATIVE, nil, &gErr)
	if gErr != nil {
		return fmt.Errorf("attach target error: %s", C.GoString(gErr.message))
	}
	...
	if gErr != nil {
		return fmt.Errorf("create script error: %s", C.GoString(gErr.message))
	}
	...
	if gErr != nil {
		return fmt.Errorf("load script error: %s", C.GoString(gErr.message))
	}
